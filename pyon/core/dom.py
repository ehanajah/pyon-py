...
class NodeList(Protocol):
    ...
    def forEach(self, callback: Callable[[Node, int, NodeList], Any]) -> None: ...
...
class Node(Protocol):
    """Base protocol for all DOM nodes (Element, Text, Comment, etc.).
    
    Mirrors the Web API Node interface. Both DOMElement and DOMTextNode
    ...
    def insertBefore(self, new_node: Node, reference: Node | None) -> Node: ...
...
class DOMTextNode(Node, Protocol):
    """Protocol for a DOM Text Node."""


class DOMElement(Node, Protocol):
