...
def _load_env(env_file: str = ".env") -> dict[str, str]:
    """
    Read .env file from root project
    Only return variables with PYON_ prefix to expose to browser
    """
    ...
    return {
        ...
        if k.startswith("PYON_")
    }


def _get_env_file() -> str:
    pyon_env = os.getenv("PYON_ENV", "development")
    candidates = [
        f".env.{pyon_env}.local",
        f".env.{pyon_env}",
        ".env.local",
        ".env",
    ]
...
def _get_dependencies_from_toml() -> list[str]:
    """Get a list of dependencies from pyon.toml"""
    ...
    try:
        with open(toml_path, "rb") as f:
            data = tomllib.load(f)
            return data.get("dependencies", {}).get("packages", [])
    except Exception as e:
        print(f"Warning: Failed to read pyon.toml: {e}")
        return []
...
def _get_dev_config() -> dict:
    ...
    try:
        with open(toml_path, "rb") as f:
            return tomllib.load(f).get("dev", {})
    except Exception:
        return {}

def _get_lock_packages() -> dict[str, str]:
...
# Available as: from pyon.env import env
try:
...
`);
        ...
        if (Object.keys(LOCK_PACKAGES).length > 0) {{
            setStatus("Installing local packages...");
        ...
        // Inject environment variables into pyon.env
        setStatus("Injecting environment variables...");
...
async def handle_packages(request: web.Request) -> web.Response | web.FileResponse:
    """Serve local packages from PROJECT_ROOT/packages_cache."""
